...
5. **2026-08-28 — Cases index legibility and end-of-case navigation.** Evidence: owner review of the five-case listing — the cases read as one block rather than five entries, and reaching the index from a case required the browser's back button. Two changes, both inside the frozen grammar. (a) In the listing, the metric column's own rules are removed and each case carries an ordinal marker, so the separator between cases is the only horizontal rule in the section and the count is legible at a glance; row padding widens to 56/72px. The ordinal is the numeral treatment already used for solutions, highlights and the market-entry process — no new element. (b) A navigation band closes each case before the CTA: previous case, index, next case, on a hairline rule over `paper` so the surface still alternates into the mist CTA. Neighbours are named by ordinal and category, not by title — case titles are full sentences, and two of them set three lines deep at each end made the band a wall of small bold text. The back-pointing arrow mirrors the existing arrow nudge rather than introducing a behaviour. Storyboard §5's detail template gains this beat. No new motion class, no new decorative element, no card, no token change. Whole-page impact: none outside the cases routes.
